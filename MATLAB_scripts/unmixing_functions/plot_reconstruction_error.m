function [err_min, err_max]= plot_reconstruction_error(X, X_hat, width, height, provide_bounds, tot_nb, nb, type)
    ...
    %       tot_nb         - (Optional) Total number of subplots. Default is 1.
    %       nb             - (Optional) Index of the current subplot. Default is 1.
    ...
    if strcmp(type, 'sam')
        err = 180 / pi * acos(dot(X, X_hat, 2) ./ (vecnorm(X, 2, 2) .* vecnorm(X_hat, 2, 2)));
