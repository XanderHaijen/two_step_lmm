function [X, A, S, E0, parameters, err] = generateSyntheticImage(height, width, p, k, varargin)
    % Generate a synthetic hyperspectral image according to the specified parameters
    % [X, A, S, E0, PARAMETERS, ERR] = GENERATESYNTHETICIMAGE(HEIGHT, WIDTH, P, K, VARARGIN)
    %
    % REQUIRED PARAMETERS
    ...
    %
    % OUTPUT
    %   X = the synthetic hyperspectral image (p x height*width)
    %   A = the abundance maps (k x height*width)
    %   S = the scaling factors (size depends on the model)
    %   E0 = the uncorrupted endmembers (p x k)
    ...
    % select k endmembers
    E0 = endmembers(1:k, wavelengths)';
    if em_noise > 0
        E = E0 + em_noise * randn(p, k);
    else
        E = E0;
    end
